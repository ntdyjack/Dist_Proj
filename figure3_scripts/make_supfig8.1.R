...
res <- sapply(dists_vec, function(x) sapply(datasets_vec, function(y) {
  tmp <- readLines(paste0(results_dir,'splatsim_',y,'_hvg1k.mtx.',x,".kmeds.txt"))
...
pdf("/users/ndyjack/Dist_Proj/figures_final/supfig_8.1.pdf",width=8,height=12)
...
write.csv(res,"/users/ndyjack/Dist_Proj/tables_final/suptab_10.1.csv",quote=F)
