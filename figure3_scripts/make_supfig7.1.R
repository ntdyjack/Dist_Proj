...
res <- sapply(dists_vec, function(x) sapply(datasets_vec, function(y) {
  tmp <- readLines(paste0(results_dir,'splatsim_',y,'_hvg1k.mtx.',x,".k50.txt"))
...
xlim <- c(-0.5,nc+1)
ylim <-c(-0.5,nr+0.3)
zleglocs <- c(0.01,0.20,0.40,0.60,0.80,0.99)
zleglabs <- c("0.15","0.32","0.49","0.66","0.83","1.00")
txtcut <- 0.30
...
pdf("/users/ndyjack/Dist_Proj/figures_final/supfig_7.1.pdf",width=8,height=12)
 ...
 #draw heatmap
  ...
  mtext(side=3,text='kAccuracy', cex=1.2)

dev.off()

rownames(res) <- paste0("splatsim_",rownames(res))
write.csv(res,"/users/ndyjack/Dist_Proj/tables_final/suptab_9.1.csv",quote=F)
