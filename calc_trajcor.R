...
library(pacman)
p_load(Matrix,Rfgc,slingshot)

#args <- c("/users/ndyjack/Dist_Proj/rdata/","/users/ndyjack/Dist_Proj/tables/test_datasets/","rnamixss_rnaseq_traj_hvg500","0","trajcor_results_July2020.txt")
...
cat("\ncalculating MDS...\n")
mds_mat <- as.matrix(cmdscale(d=dist_mat,k = 10))

cat("\ncalculating slingshot...\n")
cors_by_traj <- sapply(1:ncol(traj_lab), function(i) {
  use <- which(!is.na(traj_lab[,i]))
  slingshot_res <- slingshot(data=mds_mat[use,],clusterLabels=traj_lab[use,i],start.clus=min(traj_lab[use,i]),end.clus=max(traj_lab[use,i]))
  lineages <- getLineages(slingshot_res)
  curves <- getCurves(lineages)
  pseudotime_cors <- apply(slingPseudotime(curves),2,function(x) cor(x,traj_lab[use,i],use='complete.obs',method='pearson'))
  return(max(abs(pseudotime_cors)))
})

output <- data.frame(pearson=cors_by_traj,trajectory=colnames(traj_lab)) #paste0(args[3],".",args[4]," ",cors_by_traj)
outfile <- paste0(sub(x=args[2],pattern="test_datasets","trajcor"),args[3],".",args[4],".txt")
