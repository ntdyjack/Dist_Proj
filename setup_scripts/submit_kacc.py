...
r_script = '/users/ndyjack/Dist_Proj/scripts/calc_kacc.R '
...
for x in fnames:
  for i in idx_test:
    arg_tmp =  x + ' ' + str(i) + ' '
    name_tmp = x + '.' + str(i) 
    args = sh_script + r_script + data_dir + res_dir + arg_tmp + k
    err = '-e ' + out_dir + name_tmp  + '.kacc.err '
    out = '-o ' + out_dir + name_tmp + '.kacc.out '
