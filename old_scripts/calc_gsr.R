...
#args <-c("/users/ndyjack/Dist_Proj/pickles/","/fastscratch/myscratch/ndyjack/pickles/","/users/ndyjack/Dist_Proj/tables/gsr/","full","0")
...
ws_true <- calc_w(dist_mat,lab_dir)
...
ws_bg <- sapply(1:n_sim, function(i) {
  ...
  dist_mat <- as.dist(dist_mat)
  ws_tmp <- calc_w(dist_mat)
  return(ws_tmp)
})

gap_stats <- sapply(1:2, function(k) mean(log(ws_bg[k,])) - log(ws_true[k]) )
