...
sim_files <- sim_files[grep("jurkat293t.nC.[0-9]",sim_files)]
sim_files <- sim_files[order(as.numeric(sapply(sim_files, function(x) strsplit(x,"\\.")[[1]][3])))]
sim_files <- paste0(in_dir, sim_files)
sim_results <- unlist(lapply(sim_files, function(x) readRDS(x)),recursive=F)

#plotting parameters
nC_vals <- sapply(sim_results, function(x) x$nCells)
ord <- order(nC_vals)
nC_vals <- nC_vals[ord]
xmin <- min(nC_vals)
xmax <- max(nC_vals) 
...
plotdat_sil <- lapply(c('euc','poi','lrm'), function(x) {
  t(sapply(sim_results[ord], function(y) quantile(y$sil[,'euc'],qtls)))
#  do.call(rbind,lapply(sim_results[ord], function(y) t(sapply(y, function(z) quantile(z$sil[,x],qtls)))))
})
plotdat_ari <- lapply(c('euc','poi','lrm'), function(x) {
  sapply(sim_results[ord], function(y) y$ari[x])
#  unlist(lapply(sim_results[ord], function(y) sapply(y, function(z) z$ari[x])))
})
plotdat_time <- lapply(c(1,3,2), function(x) {
  sapply(sim_results[ord], function(y) y$time[x])
#  unlist(lapply(sim_results[ord], function(y) sapply(y, function(z) z$time[x])))
})
...
pdf("/users/ndyjack/Dist_Proj/images/jurkat293t_nC_tests2.pdf",width=8,height=6)
...
for(i in 1:3){
  lines(x=nC_vals,y=plotdat_sil[[i]][,2],lty=1,col=cols[i],lwd=1)
  points(x=nC_vals,y=plotdat_sil[[i]][,2],pch=16,col=cols[i],cex=1)
  polygon(x=c(nC_vals,rev(nC_vals)), 
...
for(i in 1:3){
  lines(x=nC_vals,y=plotdat_ari[[i]],lty=1,col=cols[i],lwd=1)
  points(x=nC_vals,y=plotdat_ari[[i]],pch=16,col=cols[i],cex=1)
}
...
for(i in 1:3){
  lines(x=nC_vals,y=plotdat_time[[i]],lty=1,col=cols[i],lwd=1)
  points(x=nC_vals,y=plotdat_time[[i]],pch=16,col=cols[i],cex=1)
}
dev.off()
